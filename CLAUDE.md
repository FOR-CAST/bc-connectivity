...
`tar_manifest()` is the check: it builds the whole pipeline definition, so it catches what `tar_validate()` catches, and the target count is a useful regression signal in itself -- dataprep is 112 for every district and each omniscape project is 16.
