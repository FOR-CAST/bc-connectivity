...
# --- Set working directory to TEAMs ---

setwd("C:/Users/CWILTSE/OneDrive - Government of BC/External_ Landscape Integrity - Downloaded Data")
...
# Write shapefile
st_write(Quesnel_TSA_boundary, "QuesnelTSA_studyarea.shp", delete_layer = TRUE)
...
writeRaster(dem_mask, "Quesnel_TSA_DEM.tif", overwrite = TRUE)
...
datasets <- list(
  list(id = "1b30f3bd-0ad0-4128-916b-66c6dd91dea4", select_cols = c("OGMA_TYPE", "LEGAL_OGMA_PROVID", "FEATURE_AREA_SQM", "FEATURE_LENGTH_M", "OBJECTID"), out = "OGMAcurrent.shp"),
  list(id = "f7dac054-efbf-402f-ab62-6fc4b32a619e", select_cols = c("GNIS_NAME_1"), out = "Rivers.shp"),
  list(id = "93b413d8-1840-4770-9629-641d74bd1cc6", select_cols = c("WATERBODY_KEY"), out = "Wetlands.shp"),
  list(id = "cb1e3aba-d3fe-4de1-a2d4-b8b6650fb1f6", select_cols = c("WATERBODY_KEY", "AREA_HA"), out = "Lakes.shp"),
  list(id = "f358a53b-ffde-4830-a325-a5a03ff672c3", select_cols = c("BGC_LABEL", "ZONE", "SUBZONE", "VARIANT", "NATURAL_DISTURBANCE"), out = "BEC.shp"),
  list(id = "b19ff409-ef71-4476-924e-b3bcf26a0127", select_cols = c("COMMON_SPECIES_NAME", "TIMBER_HARVEST_CODE", "FEATURE_AREA_SQM"), out = "WHAs.shp"),
  list(id = "1130248f-f1a3-4956-8b2e-38d29d3e4af7", select_cols = c("PARK_CLASS", "PROTECTED_LANDS_DESIGNATION", "FEATURE_AREA_SQM"), out = "BCParks.shp"),
  list(id = "4ff93cda-9f58-4055-a372-98c22d04a9f8", select_cols = c("TRACK_NAME"), out = "Railways.shp"),
  list(id = "a60d7b6e-88b2-4105-95e2-aaf6cc3468cf", select_cols = c("TIMBER_HARVEST_OPP_CODE"), out = "MDWR.shp"),
  list(id = "c58a54e5-76b7-4921-94a7-b5998484e697", select_cols = c("FIRE_YEAR", "BURN_SEVERITY_RATING"), out = "Burn_severity.shp"),
  list(id = "11277e35-d8be-47e4-bb1f-c38e393179c6", select_cols = c("FEATURE_AREA_SQM", "LANDSCAPE_UNIT_NAME"), out = "Landscape_units.shp")
)
...
st_write(Moose_wetlands, "Moose_wetlands.shp", delete_layer = TRUE)
...
streamsorder <- st_read("H:/Phase3/DownloadedData/Stream Network/BCGW_02001F02_1750206024731_7672/FWA_STREAM_NETWORKS_SP/FWSTRMNTWR_line.shp")
streamsorder <- streamsorder %>%
  st_intersection(Quesnel_TSA_boundary)
st_write(streamsorder, "streamsorder.shp", delete_layer = TRUE)
...
to <- "Cariboo_Consolidated_Roads.gdb"
dir.create(to)
file.copy(list.files(from, full.names = TRUE), file.path(to, basename(list.files(from))), overwrite = TRUE)
roads <- st_read("H:/Phase3/DownloadedData/Cariboo_Consolidated_Roads.gdb")
roads_clipped <- st_intersection(roads, Quesnel_TSA_boundary)
Roads_filtered <- roads_clipped %>% select(TRANSPORT_LINE_TYPE_CODE, TRANSPORT_LINE_TENURE_TYPE_CODE)
...
st_write(Roads_filtered, "Consolidated_roads.shp", delete_layer = TRUE)
...
vri_subset <- st_read(
  dsn = "H:/Phase3/DownloadedData/VRI_Age/VEG_COMP_LYR_R1_POLY_2023.gdb/VEG_COMP_LYR_R1_POLY_2023.gdb",
...
Human_disturbance_subset <- st_read(
  dsn = "H:/Phase3/DownloadedData/Human_disturbance/New folder/BC_CEF_Human_Disturbance_2023/BC_CEF_Human_Disturbance_2023/BC_CEF_Human_Disturbance_2023.gdb",
...
names(Human_diturbance_selected)[names(Human_diturbance_selected) == "CEF_DISTURB_GROUP"] <- "Dstrb_Grp";
names(Human_diturbance_selected)[names(Human_diturbance_selected) == "CEF_DISTURB_SUB_GROUP"] <- "Dstrb_Subg"
names(Human_diturbance_selected) <- make.unique(substr(names(Human_diturbance_selected), 1, 10), sep = "_")
st_write(Human_diturbance_selected, "Human_disturbance.shp", delete_layer = TRUE)
...
ForestDisturbance_Quesnel <- st_read(
  dsn = "C:/Users/CWILTSE/OneDrive - Government of BC/External_ Landscape Integrity - Downloaded Data/BC_CEF_Forest_Disturbance_2024.gdb",
  layer = "BC_CEF_ForestDisturbance_2024",
  wkt_filter = st_as_text(st_as_sfc(bbox))
)
st_write(ForestDisturbance_Quesnel, "Forest_Disturbance.shp", delete_layer = TRUE)
...
landcover <- raster("H:/Phase3/DownloadedData/landcover-2020-classification.tif")
...
writeRaster(landcover_bc, "BC_landcover.tif", overwrite = TRUE)
