...
targets::tar_read(interpatch_distances, branches = 1) |>
  unlist() |>
  quantile(seq(0, 1, 0.05))
```

We selected the 20th percentile value of all interpatch distances as the maximum search radius (`r targets::tar_read(interpatch_distances, branches = 1) |> unlist() |> quantile(0.2) |> prod(0.001) |> round(1)` km).

**Nearest neighbour interpatch distances (quantiles)**

```{r nn_distances, echo = FALSE}
targets::tar_read(interpatch_distances, branches = 2) |>
  unlist() |>
  quantile(seq(0, 1, 0.05))
```

We selected the 100th percentile value of the nearest neighbour interpatch distances as the minimum search radius (`r targets::tar_read(interpatch_distances, branches = 2) |> unlist() |> quantile(1) |> prod(0.001) |> round(1)` km).
