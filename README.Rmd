...
source("scripts/workflow_seral_overview.R")
```

![Overview of seral stage workflow](workflow_seral_overview.png)

```{r workflow-seral-detailed, echo = FALSE}
source("scripts/workflow_seral_detailed.R")
```

![Detailed seral stage classification](workflow_seral_detailed.png)

Seral stage patch calculations adapted from BC Gov `arcpy` scripts:

1. `get_input_data` loads the seral stage polygons:
  a. loads `r1_seral_managed` polygon layer, saving as `seral_managed`;
  b. loads `aoi` (LU) study area polygon(s);
  b. takes `seral_managed` polygon layer and repairs geometry, then dissolves polygons (`SINGLE_PART`) based on attributes `SERAL_STAGE` and `early_less20yrs`, saving as `seral` polygon layer;
