...
Two defects in `terra`'s overlay operators were found while rerunning the pipeline, and both were reported upstream.
Neither produces an error: they return plausible-looking wrong geometry.
...
Neither defect reproduces on synthetic geometry, so a passing unit test is not evidence that either operator is safe on real data.

**The Omniscape radii change**, which is the correction that propagates furthest.
Radii are derived from quantiles of the interpatch distance distribution, and that distribution is measured between patches whose construction the 2026-08-25 corrections changed:
...
Both move by more than one pixel at either resolution, so every published Omniscape run needs redoing with a new configuration.
The corrected configurations are stamped `2026-08-26`.

`patches_union_final` is now tiled over `study_area_tiles`, which cuts it from **5 h 51 m 58 s to 17 m 48 s** (19.8x).
Cropping splits polygons at the tile seams, so `forest_patches_final.gpkg` carries **192,683 features against 189,666** (+1.59%).
No area moves: the tiled and serial layers agree to **0.000000 ha**, and to sub-mm^2 on every combination of `Seral`, `matold_interior` and `old_interior`.
The split does not reach the distance calculations, because `calc_matold()` dissolves first -- serial and tiled nearest-neighbour quantiles agree to under a metre at every level, and the q100 that sets the radius is identical.
