...
# GitHub-flavoured markdown, which is what this document is FOR. The previous setup rendered an
# `html_document` and kept knitr's intermediate markdown as `README.md`, so the file GitHub shows
# was the one pandoc had never processed: every citation appeared on the front page as a raw
# `[@key]` and the reference list was never generated at all. `gfm` writes the markdown directly,
# through citeproc, so what is committed is what was rendered.
format:
  gfm:
    toc: false
bibliography: citations/references.bib
# `csl:`, not `citation-style:` -- the latter is not a field pandoc or rmarkdown reads, so this
# stylesheet had never been applied to anything.
...
## Load the project's functions. This document used to be rendered only as a pipeline target, where
## `tar_source()` had already run, so the chunk calling `get_path()` below worked by inheriting that
## session. Rendering it directly has no such context, and direct rendering is now the only way it
## happens: the only `tar_render()` for it lived in the retired `main` pipeline.
targets::tar_source()
```
...
The pipeline runs once per Natural Resource District, as a pair of projects -- a `dataprep` project
ending at the Omniscape inputs, and an `omniscape` project that consumes them.
`_targets.yaml` names them; `TAR_PROJECT` selects one.
...
Substitute `chilcotin` or `hundred_mile` for `quesnel` to run another district.
Each district's inputs and results live under its own key -- `Data/processed/<district>/`,
`Data/processed/rasters/<district>/`, `Outputs/<district>/` -- so districts never overwrite one
another.

There is no default project: `TAR_PROJECT` is required.
A bare `targets::tar_make()` resolves to `_targets.R`, which is the retired single-district Quesnel
pipeline and refuses to run.
...
By default the `dataprep` project prepares every Omniscape input, and the `omniscape` project stops short of launching the runs themselves, which take hours to days each.
...
The province-wide layers are identical for every district, so they are fetched once into `Data/raw/` by the `shared_inputs` target rather than by each step that reads them.
That directory is shared, and on a setup where it is on network storage it is shared between machines too, so the fetch is written to be safe to run from several districts and several hosts at the same time: one caller downloads while the rest wait, and each file is only moved to its final name once it is complete.
Running the three districts concurrently therefore needs nothing set up beforehand -- a bare `tar_make()` on each is enough.
...
Omniscape runs are launched from the `targets` workflow, so a district's whole analysis is two `tar_make()` calls.
...
```{r omniscape-benchmarks, echo = FALSE, message = FALSE}
## Measured resource use, written by `omniscape_benchmark_table()` from the `time -v` figures each
## run records. Falls back to a note when no runs have been made in this store.
##
## Resolved through `get_path()` rather than written out: the writer uses it too, and it now files
## outputs under the district key. A literal `Outputs/` here would look in the wrong place and show
## the fallback note for ever, which is indistinguishable from having made no runs.
bench <- file.path(get_path("outputs"), "omniscape_benchmarks.csv")
if (file.exists(bench)) {
  knitr::kable(read.csv(bench))
} else {
  cat("_No Omniscape runs recorded in this store; set `BC_CONN_OMNISCAPE` and rerun._\n")
}
```
...
This also puts a version-suffixed `Rscript-4.5.3` on the path, which is what the commands above use.
A bare `Rscript` resolves to whichever R comes first on the path, and `renv/library/` is per R minor
version, so under a different one the project library looks completely empty and every command fails
as though the project itself were broken.
