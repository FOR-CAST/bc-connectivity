...
# `[@key]`, the reference list was never generated at all, and this YAML block was printed above
# the title. `gfm` writes the markdown directly, through citeproc, so what is committed is what
# was rendered.
format:
  gfm:
    toc: false
...
## Load the project's functions. As a `tar_quarto()` target this document inherits a session where
## `tar_source()` has already run, so the chunk calling `get_path()` below worked by inheritance.
## Rendering it directly -- which is how `README.md` gets refreshed in practice -- has no such
## context, and the chunk fails with "could not find function".
targets::tar_source()
```
